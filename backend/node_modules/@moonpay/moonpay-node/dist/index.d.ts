...
type SellTransactionStatus = 'completed' | 'failed' | 'pending' | 'waitingForDeposit';
/**
 ...
 * lifecycle events.
 * */
interface RampsChildToParentInterface extends RampsSwapsKycChildToParentInterface, RampsConsumerKycChildToParentInterface {
    /**
...
interface AdvancedWidgetQueryParams {
     ...
     * Possible values are ach_bank_transfer, credit_debit_card, gbp_bank_transfer, gbp_open_banking_payment, mobile_wallet, sepa_bank_transfer, sepa_open_banking_payment and pix_instant_payment.
...
interface FiatBalanceWidgetQueryParams extends ThemeQueryParams {
    /**
     * Configures the widget origin instead of inferring it from the environment.
     */
    baseOrigin?: string;
}
type FiatBalanceDepositWidgetQueryParams = FiatBalanceWidgetQueryParams;
type FiatBalanceWithdrawWidgetQueryParams = FiatBalanceWidgetQueryParams;
/**
 * @deprecated
 */
...
type MoonpayCoreSdkFiatBalanceDepositConfig = MoonPayCoreSdkConfigCommon<'fiatBalanceDeposit', FiatBalanceDepositWidgetQueryParams>;
type MoonpayCoreSdkFiatBalanceWithdrawConfig = MoonPayCoreSdkConfigCommon<'fiatBalanceWithdraw', FiatBalanceWithdrawWidgetQueryParams>;
...
type MoonPayCoreSdkConfig = MoonPayCoreSdkBuyConfig | MoonPayCoreSdkSellConfig | MoonPayCoreSdkSwapsCustomerSetupConfig | MoonPayCoreSdkNftConfig | MoonPayCoreSdkLegacyConfig | MoonPayCoreSdkClaimConfig | MoonPayCoreSdkTopUpConfig | MoonPayCoreSdkWithdrawConfig | MoonPayCoreSdkSwapConfig | MoonPayCoreSdkTransactionTrackerConfig | MoonpayCoreSdkFiatBalanceDepositConfig | MoonpayCoreSdkFiatBalanceWithdrawConfig | MoonPayCoreSdkConsumerKycConfig;

type MoonPayURLAuth = MoonPayCoreSdkConfig['auth'];
type MoonPayOptions = {
    debug: boolean;
};
type ExtractFlow<T, F extends WidgetFlow> = T extends {
    flow: F;
} ? T : never;
type ExtractedConfig<T extends WidgetFlow> = ExtractFlow<MoonPayCoreSdkConfig, T>;
type MoonPayWidgetQueryParams<T extends WidgetFlow> = ExtractedConfig<T>['params'];
...
declare class MoonPayURL {
     ...
     */
    generateSignature(url: string, { returnFullUrl: returnFullURL }?: GenerateSignatureOptions): string | null;
    /**
     * Verify that a signed URL is valid.
     * @param url The URL to verify.
     * @returns `true` if the URL is valid, otherwise `false`.
     */
    isSignatureValid(urlString: string): boolean;
    /**
     ...
     * });
     */
    generate<TFlow extends WidgetFlow>({ flow, params, auth, }: {
        flow: TFlow;
...
declare class MoonPay {
    url: MoonPayURL;
    constructor(secretKey: string, { debug }?: MoonPayOptions);
}
